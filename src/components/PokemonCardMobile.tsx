...
function PokemonCardMobile(props: any) {
  ...
  const handleModalClose = () => {
    // Set animation state to closed
    setIsOpenAnimation(false);
    // Insert timer to wait for animation to finish
    setTimeout(() => {
  ...
  return (
    ...
    <div className="fixed right-0 left-0 bottom-0">
        ...
        <motion.div
          ...
          variants={variants}
          className="modal bg-white shadow-lg rounded-xl overflow-hidden h-full"
        >
          {/* If type is normal fetch next type */}
          <div
            className={`p-40 ${constants.default[Pokemon.types[0].type.name]}`}
          ></div>
          <div className="flex justify-end">
            {/* Create cross circle button */}

            <button
              className="relative p-4 m-4 bg-slate-200 rounded-full "
          ...
          <div className="bg-white rounded-xl">
            ...
            <div className="rounded-lg shadow-lg p-4 ">
              <p className="text-gray-400 text-center">#{props.PokemonId}</p>
                  ...
                  {Pokemon.abilities.map((ability, idx) => (
                    <div
                      className={` w-40 flex justify-center py-1 border rounded-2xl bg-slate-100 p-2 ${
                        ability.is_hidden ? "border-red-700" : "border-blue-300"
                      } `}
                    >
              ...
              <div className={`pt-4 flex flex-col justify-around text-center`}>
